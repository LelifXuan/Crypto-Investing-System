        ...
        if p.is_dir() and name in SAFE_GENERATED_DIRS:
            ...
            fp.append(
                Finding(
                    ...
                    "generated_cache",
                    rel,
                    f"Generated cache ({human_size(sz)}).",
                    "Safe to delete.",
                    sz,
                    True,
                )
        ...
        if p.exists() and p.is_dir():
            ...
            fp.append(
                Finding(
                    ...
                    "runtime_artifact",
                    dn,
                    f"Runtime dir ({human_size(sz)}).",
                    "Delete for release builds.",
                    sz,
                    True,
                )
            )
    ...
    if venv.exists():
        fp.append(
            Finding(
                ...
                "release_bloat",
                ".venv",
                f"Virtual env ({human_size(path_size(venv))}).",
                "Do not ship. Keep in gitignore.",
                path_size(venv),
                False,
            )
        )
...
def safe_clean(repo: Path, findings: Sequence[Finding]) -> list[str]:
    removed = []
    for f in findings:
        if not f.auto_cleanable:
            continue
        p = repo / f.path
        if not str(p.resolve()).startswith(str(repo.resolve())):
            continue
        if any(part in NEVER_DELETE for part in p.relative_to(repo).parts):
            continue
        try:
            if p.is_dir():
                shutil.rmtree(p)
...
def main():
    ...
    report = {
        ...
        "summary": {
            ...
            "auto_cleanable_human": human_size(
                sum(f.size_bytes for f in findings if f.auto_cleanable)
            ),
            "removed": len(removed),
        },
        "removed": removed,
    ...
    print(
        ...
        f"Auto-cleanable: {report['summary']['auto_cleanable_human']} | "

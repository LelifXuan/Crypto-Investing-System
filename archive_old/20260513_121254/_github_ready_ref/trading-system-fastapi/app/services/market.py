...
class MarketService:
    ...
    async def fetch_and_persist_live_mark(self, instrument_id: str) -> MarkPrice:
        ...
        if ref.product_type == "spot":
            ...
            contract = await self.gate_client.get_futures_contract(ref.settle or settings.gateio_default_settle, ref.symbol)
            model = MarkPrice(
                instrument_id=instrument.instrument_id,
    ...
    async def sync_candles_from_provider(
        ...
        candles = [
            MarketCandle(
                ...
                close=item.close,
                volume=item.volume,
                source=item.source if price_kind == "last" else f"{item.source}:{price_kind}",
            )
            for item in remote
        ]
        if not persist:
    ...
    async def add_market_event(self, event: MarketEvent, instrument_ids: list[str]) -> MarketEvent:
        ...
        if instrument_ids:
            await self.repository.add_market_event_links(
                [MarketEventInstrument(event_id=event.event_id, instrument_id=inst) for inst in instrument_ids]
            )
        return persisted
    ...
    async def _require_instrument(self, instrument_id: str) -> Instrument:
        ...
        if instrument is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"instrument not found: {instrument_id}")
        return instrument
    ...
    def resolve_gate_reference(instrument: Instrument) -> GateMarketRef:
        ...
        product_type = str(gate_meta.get("product_type") or MarketService._infer_product_type(instrument.asset_class)).lower()
        symbol = str(gate_meta.get("currency_pair") or gate_meta.get("contract") or gate_meta.get("symbol") or instrument.symbol)
        symbol = MarketService.normalize_gate_symbol(symbol)
        settle = str(gate_meta.get("settle") or instrument.settle_ccy or settings.gateio_default_settle).lower()

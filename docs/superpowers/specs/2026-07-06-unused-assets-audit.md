...
- 🟢 **可清理 ~6,600 行代码**（~12% 服务端 + ~47% CSS）— 零外部引用，确定是死代码或前代 UI 残留
...
最初 4 重引用证据判定法：
- A. CSS/Python 是否有定义
- B. JS/HTML 静态 `class="..."` 引用
- C. 模板字符串动态拼接
- D. 后端响应字段返回

**用户反馈**："某些单独的模组也是有用的，譬如宏观日历，虽然它不被复用也不影响权重"。

→ 补充**产品模块判定**（E 维度）：

- **E1 完整性**：类是否共同组成完整可视化组件（panel/head/body/empty/grid 齐备）？
- **E2 独立性**：能否脱离其他同类独立运行？
- **E3 意图清晰**：类名是否能推断要做什么？

**最终判定表**：

| 类别 | B/C/D 引用 | E 模块 | 替代关系 | 处置 |
|---|---|---|---|---|
| 🟢 真可删 | 0 | 0 (碎片) | — | 删除（历史残留） |
| 🟢 真可删 | 0 | 完整 | 已被 V3 100% 替代 | 删除（命名迁移） |
| 🟠 产品模块 | 0 | 完整 | 无替代 | **保留**（询问用户） |
| 🟡 保护 | ≥1 | — | — | 保留 |
| 🔴 边界 | 0 | 部分 | 难判断 | 询问用户 |

**应用示例**：
- `.macro-month-grid` + `.macro-month-day` + ...：B=0，E=完整，**但被 V3 `.calendar-grid` 100% 替代** → 🟢 真可删
- `.calendar-day` + `.calendar-grid`：B=有引用 → 🟡 保护
- `.btc-card-span-4`：D=后端 chart_builder.py 注入 → 🟡 保护

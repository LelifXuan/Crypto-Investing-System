...
function vwap(highs, lows, closes, volumes, window = 50) {
  const output = new Array(closes.length).fill(null);
  let weightedSum = 0;
  let volumeSum = 0;
  const weighted = closes.map((close, index) => ((highs[index] + lows[index] + close) / 3) * (volumes[index] || 0));
  for (let index = 0; index < closes.length; index += 1) {
    weightedSum += weighted[index];
    volumeSum += volumes[index] || 0;
    if (index >= window) {
      weightedSum -= weighted[index - window];
      volumeSum -= volumes[index - window] || 0;
    }
    if (index + 1 >= window && volumeSum > 0) {
      output[index] = weightedSum / volumeSum;
    }
  }
  return output;
}
...
function cci(highs, lows, closes, period = 20) {
  ...
  for (let i = 0; i < typical.length; i++) {
    ...
    const mean = sum / count;
    const absDev = Math.abs(typical[i] - mean);
    if (ringCount >= period) devSum -= ring[ringIdx];
    ring[ringIdx] = absDev;
    ringIdx = (ringIdx + 1) % period;
    devSum += absDev;
    if (ringCount < period) ringCount++;
    const meanDev = devSum / (ringCount || 1);
...
function classifyEmaRegime({ closes, ema30, ema60, ema120, atrValues }) {
  const c = closes.at(-1) || 0;
  const e30 = ema30.at(-1) || 0;
  const e60 = ema60.at(-1) || 0;
  const e120 = ema120.at(-1) || 0;
  const bullishOrder = e30 > e60 && e60 > e120;
  const bearishOrder = e30 < e60 && e60 < e120;
  const emaMax = Math.max(e30, e60, e120);
  const emaMin = Math.min(e30, e60, e120);
  const spreadPct = c > 0 ? ((emaMax - emaMin) / c) * 100 : 0;
  const atr = atrValues?.at(-1) || Math.abs(c - e30);
  const distAtr = atr > 0 ? Math.abs(c - e30) / atr : 0;
  const distPct = c > 0 ? Math.abs((c - e30) / c) * 100 : 0;
  const extended = distAtr >= 1.5 || distPct >= 3.0;

  if (bullishOrder) {
    if (extended) return { state: "risk_hot", tone: "event", label: "多头延续但追高风险", summary: `EMA 多头排列 · 价格偏离 EMA30 ${distPct.toFixed(1)}% (${distAtr.toFixed(1)}x ATR)，趋势延续但追价风险上升。` };
    if (spreadPct >= 0.8) return { state: "strong_bullish", tone: "bullish", label: "多头排列", summary: `EMA 多头排列 · 均线间距 ${spreadPct.toFixed(2)}%，趋势结构健康。` };
    return { state: "bullish", tone: "bullish", label: "多头排列", summary: `EMA 多头排列 · 均线间距偏窄 ${spreadPct.toFixed(2)}%，方向向上但动能温和。` };
  }
  if (bearishOrder) {
    if (extended) return { state: "risk_cold", tone: "event", label: "空头延续但追空风险", summary: `EMA 空头排列 · 价格低于 EMA30 ${distPct.toFixed(1)}%，反弹仍受压制。` };
    if (spreadPct >= 0.8) return { state: "strong_bearish", tone: "bearish", label: "空头排列", summary: `EMA 空头排列 · 均线间距 ${spreadPct.toFixed(2)}%，空头结构清晰。` };
    return { state: "bearish", tone: "bearish", label: "空头排列", summary: `EMA 空头排列 · 均线间距偏窄，空头结构但动能温和。` };
  }

  if (spreadPct <= 0.6) return { state: "neutral", tone: "neutral", label: "均线纠缠", summary: `EMA 均线纠缠 · EMA30/60/120 间距仅 ${spreadPct.toFixed(2)}%，方向优势不足，等待收盘脱离成本带。` };
  return { state: "neutral", tone: "neutral", label: "均线交错", summary: `EMA 均线交错 · 排列尚未形成一致，需等待方向确认。` };
}

function signalTone(text) {
  if (text.startsWith("强趋势偏多") || text.startsWith("多头增强") || text.startsWith("偏多") || text.startsWith("中性偏多") || text.startsWith("强势扩张") || text.startsWith("超卖修复") || text.startsWith("趋势成形偏多") || text.startsWith("金叉")) return "bullish";
  if (text.startsWith("强趋势偏空") || text.startsWith("空头增强") || text.startsWith("偏空") || text.startsWith("中性偏空") || text.startsWith("弱势扩张") || text.startsWith("死叉")) return "bearish";
  if (text.startsWith("超买风险") || text.startsWith("过热") || text.startsWith("事件型") || text.startsWith("多头延续但") || text.startsWith("空头延续但")) return "event";
  return "neutral";
}

function signalLabel(text) {
  if (text.startsWith("强趋势偏多")) return "强趋势偏多";
  if (text.startsWith("强趋势偏空")) return "强趋势偏空";
  if (text.startsWith("趋势成形偏多")) return "趋势成形偏多";
  if (text.startsWith("趋势成形偏空")) return "趋势成形偏空";
  if (text.startsWith("多头增强")) return "多头增强";
  if (text.startsWith("空头增强")) return "空头增强";
  if (text.startsWith("多头延续但")) return "多头偏离风险";
  if (text.startsWith("空头延续但")) return "空头偏离风险";
  if (text.startsWith("均线纠缠")) return "均线纠缠";
  if (text.startsWith("偏多")) return "偏多";
  if (text.startsWith("偏空")) return "偏空";
  if (text.startsWith("中性偏多")) return "中性偏多";
  if (text.startsWith("中性偏空")) return "中性偏空";
  if (text.startsWith("超买风险")) return "超买风险";
  if (text.startsWith("超卖修复")) return "超卖修复";
  if (text.startsWith("强势扩张")) return "强势扩张";
  if (text.startsWith("弱势扩张")) return "弱势扩张";
  if (text.startsWith("事件型")) return "事件型";
  if (text.startsWith("扩张")) return "波动扩张";
  if (text.startsWith("压缩")) return "波动压缩";
  if (text.startsWith("常态")) return "波动正常";
  if (text.startsWith("金叉")) return "金叉偏多";
  if (text.startsWith("死叉")) return "死叉偏空";
  if (text.startsWith("过热回撤")) return "过热风险";
  if (text.startsWith("超跌修复")) return "超跌风险";
  if (text.startsWith("趋势弱")) return "趋势弱";
  if (text.startsWith("放量")) return "放量";
  if (text.startsWith("量能")) return "量能平稳";
  if (text.startsWith("中性")) return "中性";
...
function volatilityInterpretation(widthPct, atrValue, close = 1) {
  const natr = close > 0 ? (atrValue / close) * 100 : 0;
  if (widthPct >= 12 || natr >= 3.5) return "事件型，波动明显扩张，仓位宜更保守。";
  if (widthPct >= 9 || natr >= 2.5) return "扩张，波动高于常态，突破和假突破概率同时上升。";
  if (widthPct <= 6 && natr <= 1.5) return "压缩，波动处于收敛区，等待方向释放。";
  return "常态，波动处于正常区间。";
}
...
function vwapInterpretation(close, vwap50, vwap100, slope10, spreadPct) {
  const has50 = Number.isFinite(vwap50);
...
function calcAnalysis(candles, bundle = null) {
  ...
  return {
    ...
    vwapValues: {
      vwap50: useSeriesOrBuild(secondary.vwap_50, () => vwap(highs, lows, closes, volumes, 50), closes.length),
      vwap100: useSeriesOrBuild(secondary.vwap_100, () => vwap(highs, lows, closes, volumes, 100), closes.length),
      spreadPct: useSeriesOrBuild(
        secondary.vwap_spread_pct,
        () => {
          const fast = vwap(highs, lows, closes, volumes, 50);
          const slow = vwap(highs, lows, closes, volumes, 100);
          return fast.map((value, index) => {
            const base = slow[index] || 0;
            return value === null || !base ? 0 : ((value - base) / base) * 100;
          });
        },
        closes.length,
      ),
      slope10: useSeriesOrBuild(
        secondary.vwap_slope_10,
        () => {
          const fast = vwap(highs, lows, closes, volumes, 50);
          return fast.map((value, index) => {
            const previous = index >= 10 ? fast[index - 10] : null;
            return value === null || previous === null || !previous ? 0 : ((value - previous) / previous) * 100;
          });
        },
        closes.length,
      ),
...
function sliceAnalysisForDisplay(analysis, limit) {
  return {
    ...
    vwapValues: {
      vwap50: sliceSeries(analysis.vwapValues?.vwap50 || [], limit),
      vwap100: sliceSeries(analysis.vwapValues?.vwap100 || [], limit),
      spreadPct: sliceSeries(analysis.vwapValues?.spreadPct || [], limit),
      slope10: sliceSeries(analysis.vwapValues?.slope10 || [], limit),
    },
...
async function loadAll(force = false) {
  ...
  try {
    ...
    const emaRegime = classifyEmaRegime({
      closes: analysis.closes,
      ema30: analysis.ema30,
      ema60: analysis.ema60,
      ema120: analysis.ema120,
      atrValues: analysis.atrValues,
    });
    ...
    const volText = volatilityInterpretation(bollWidthPct, analysis.atrValues.at(-1) || 0, close);
    ...
    const vwap50 = analysis.vwapValues?.vwap50?.at(-1);
    const vwap100 = analysis.vwapValues?.vwap100?.at(-1);
    const vwapSpread = analysis.vwapValues?.spreadPct?.at(-1) || 0;
    const vwapSlope = analysis.vwapValues?.slope10?.at(-1) || 0;
    const vwapText = vwapInterpretation(close, Number(vwap50), Number(vwap100), Number(vwapSlope), Number(vwapSpread));

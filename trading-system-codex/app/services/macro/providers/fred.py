...
class FredMacroProvider:
    ...
    async def fetch_history(
        ...
        """Return up to ``lookback_points`` ascending observations.

        The FRED official JSON API supports a ``limit`` parameter; we pull
        60 observations at once (5 years of monthly data) and trim to
        ``lookback_points`` after dropping ``.`` and empty rows.
        """
        api_key = self.secrets.get("FRED_API_KEY")
        if api_key:
            try:
                return await self._fetch_official_history(source_key, api_key, lookback_points)
            except (httpx.HTTPError, ValueError):
                return await self._fetch_public_history(source_key, lookback_points)
        return await self._fetch_public_history(source_key, lookback_points)
    ...
    async def _fetch_official_history(
        self, source_key: str, api_key: str, lookback_points: int
    ) -> list[MacroFetchPoint]:
        params = {
            "series_id": source_key,
            "api_key": api_key,
    ...
    async def _fetch_public_history(
        ...
        async with client_for_source("fred", timeout=15) as client:
            response = await client.get(
                settings.fred_public_csv_url, params={"id": source_key}
            )
            response.raise_for_status()
        ...
        for row in rows:
            ...
            if not raw_value or raw_value == ".":
                points.append(
                    MacroFetchPoint(observation_ts=observation_ts, value=None, status="missing")
                )
                continue
            try:
                numeric = D(raw_value)

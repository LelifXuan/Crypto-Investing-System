...
class AlphaVantageMacroProvider:
    ...
    async def _request(self, params: dict):
        cache_key = (
            f"av:{params.get('function')}:{params.get('symbol', params.get('from_symbol', ''))}"
        )
        if self.cache:
        ...
        start = time.time()
        async with client_for_source("alpha_vantage", timeout=30) as client:
    ...
    async def connectivity_check(self) -> dict:
        ...
        try:
            ...
            return {
                ...
                "status": "error",
                "latency_ms": int((time.time() - start) * 1000),
                "auth": self.secrets.auth_state({"ALPHA_VANTAGE_API_KEY"}),
                "error": str(exc)[:200],
            }

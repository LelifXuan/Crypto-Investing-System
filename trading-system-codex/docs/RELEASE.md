...
## V1.5.5 发布前验收清单（监控总览 6 项反馈修复）

V1.5.5 在 V1.5.4 基础上修复用户实地核查监控总览页时提出的
6 项问题：

### 改动一览

- [⑥ 结构模块] `monitoring_dashboard._load_cached_structure_payload`
  读 `payload.snapshot.overall.{overall_bias, score, regime, ...}`
  替换错误的 `payload.get("score")` 顶层读取；`StructureSummaryAdapter`
  `_BIAS_TO_IMPACT` 加 `weak_bullish / weak_bearish / mild_* /
  uncertain / no_clear_structure` 映射并加 score clamp。
  chip_structure proxy 路径强制 `confidence=0.2` + `is_proxy=True`，
  `StructureSummaryAdapter.summarize` 在 proxy 模式下短路返回
  `结构待刷新` 状态，不参与方向结论。
- [② 中性震荡亚分类] `_determine_regime` 末尾按 `global_score` 区分
  `偏多震荡` (>=55) / `偏空震荡` (<=45) / `中性震荡` (中间)。
  前端 `renderTerminalSummary` 合并 `regime` + `bias · confidence` 两个
  chip 为单个 regime chip + 置信度数字。
- [③ 文本精炼] `_generate_text` 头部从 3 子句砍到 1 句；
  `_decision_build_market_row` 4 个分支的 summary 全部砍到 1 句并去重
  regime 前缀（regime 已在 head chip 显示）。
- [④ 关键失效行重写]
  - 删 `数据缺口` bullets（系统内部数据质量报告，不应给用户看）
  - 删 summary 里罗列的「数据缺口/跨周期冲突/...」固定文案
  - summary 改为 `关键失效条件：{topmost invalidation}` 单句
  - `_decision_text` 不再 `str(dict)` repr：识别 `text / message / label
    / reason / summary` 字段；list 单元素解包；多元素 list 用 `；` 拼接
  - `_decision_pick_critical_invalidation` 返回的 `{text, source}` dict
    在 `key_risk_row` 显式 `.get("text")` 解包，不再 repr
- [⑤ source_refs chip] 新增 `SOURCE_REF_META` 字典映射每个
  source_ref key 到 `{label_cn, page_id}`。`missing:{bundle}` 走
  专有路径自动加 `（未刷新）`后缀并指向对应页面。3 个 row builder
  都在 `source_refs` 旁附 `source_refs_meta`。前端 `renderTerminalDecisionRow`
  把 chip 渲染为 `<a data-page-link="{page}" href="#{page}">`，
  复用 V1.5.4 D1 的 SPA 路由跳转。CSS 加 `:hover`/`:focus-visible`。
- [① 布局] `monitoring-snapshot-grid` 改 `grid-template-areas`:
  `terminal terminal / macro technical`，brief card 跨满 content 宽度。
  6 个 vote tile 在 brief card 内部保留（6 / 3 / 2 响应式）。
  `.monitoring-left-stack` 只包 terminal；新增
  `.monitoring-technical-stack` 包 technical。
  headline 加 `line-clamp: 3` + `max-width: 1240px`。
...
### V1.5.5 实地核查（webfetch + playwright）

- [ ] `curl -s 'http://127.0.0.1:8002/api/v1/monitoring/dashboard?instrument_id=btc-usdt-perp&timeframe=1d' | jq '.terminal_summary.module_scores.structure'`
  - [ ] 字段有 `overall_bias` 时：`impact ∈ {bullish, mild_bullish, bearish, mild_bearish}`，不再回退到 `neutral`
  - [ ] `weak_bearish` → `mild_bearish` + score `<= 48`
  - [ ] proxy 路径下：`state = "结构待刷新"`、`confidence < 0.3`
- [ ] 头部 `regime` 是 `偏多震荡` / `偏空震荡` / `中性震荡` 之一
- [ ] `decision_brief.rows[0].summary` 不再以 `当前为{regime}` / `当前维持{regime}` 开头
- [ ] `decision_brief.rows[*].source_refs_meta[*].label` 是中文（筹码结构 / 背离摘要 / 形态结构 / ...）
- [ ] 关键失效行不再出现 `数据缺口` bullets；不再出现 Python dict 字面量
- [ ] playwright: 点击 source_refs 上的「筹码结构」chip 跳转告警中心页（走 SPA 路由不刷新整页）

### 已知限制

- 当 structure / strategy / alerts / analysis bundle 全部 cache 缺失时，
  关键失效行的 chip `数据未刷新` 会指向监控总览本身（兜底），
  用户可以从那里进 precompute 队列触发 pipeline。
- `STRUCTURE_BUNDLE` 与 chip_structure 在 system_pa / config 故障场景
  下可能短暂同时缺失；这种情况 `regime` 仍能算，但 `mtf_breakdown`
  可能为空。
- ③ 的 `line-clamp: 3` 是 CSS 截断，后端没硬限制单句长度。极端
  long-text 仍会触发 max-height 后的截断提示，需要文案侧自律。

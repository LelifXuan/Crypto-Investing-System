...
DEFAULT_REQUIREMENTS = {"1h": 720, "4h": 480, "1d": 420, "1w": 260, "30d": 180, "1m": 240, "5m": 240, "15m": 240}
LONG_REQUIREMENTS = {"1h": 1000, "4h": 900, "1d": 900, "1w": 520, "30d": 360, "1m": 1000, "5m": 1000, "15m": 1000}
TTL_CACHE_TABLES = ("page_snapshot_cache", "computed_dataset_cache")
LONG_TERM_TABLES = ("market_candles", "mark_prices", "strategy_decision", "signal_outcome", "translation_text_cache", "translation_cache")
...
def kline_coverage(conn, tables):
    ...
    """).fetchall()
    items = []
    for inst, tf, src, cnt, min_ts, max_ts in rows:
        ...
        items.append({
            "instrument_id": inst, "timeframe": tf, "source": src,
            "count": int(cnt), "default_required": def_need,
...
def summarize_db(repo, db_path, active_urls):
    item = {"path": str(db_path), "rel": str(db_path.relative_to(repo)) if db_path.is_relative_to(repo) else str(db_path), "exists": db_path.exists(), "size_bytes": db_path.stat().st_size if db_path.exists() else 0, "referenced_by": [], "error": None}
    ...
    try:
        ...
        item["tables"] = {t: int(c.execute(f"select count(*) from {t}").fetchone()[0]) for t in sorted(names) if t in names}
        ...
        for t in TTL_CACHE_TABLES:
            if t in names:
                try:
                    expired += int(c.execute(f"select count(*) from {t} where expires_at is not null and expires_at < ?", (now,)).fetchone()[0])
                except: pass
        item["expired_cache_rows"] = expired
        item["recommendation"] = "Run clean_workspace.py --clear-data to clear expired cache" if expired > 0 else None
        c.close()
    ...
    for u in urls.values():
        if u.startswith("sqlite+aiosqlite:///"):
            ...
            if tp not in dbs:
                dbs.append(tp)
    dbs = sorted(set(dbs))
    report = {"generated_at": datetime.now(timezone.utc).isoformat(), "repo": str(repo), "database_urls": urls, "databases": [summarize_db(repo, p, urls) for p in dbs]}
    if args.write_runtime_log:
        ld = repo / "runtime/logs"
        ld.mkdir(parents=True, exist_ok=True)
        (ld / "local_storage_audit.json").write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    if args.format == "json":
        ...
        for db in report["databases"]:
            ...
            if db.get("error"):
                ...
                continue
            t = db.get("tables", {})
            print(f"  market_candles={t.get('market_candles',0)} cache_snapshot={t.get('page_snapshot_cache',0)} computed={t.get('computed_dataset_cache',0)}")
            deficits = db.get("deficits", [])
            print(f"  sample_deficits={len(deficits)} expired_cache={db.get('expired_cache_rows',0)}")
            for d in deficits[:20]:
                print(f"    {d['instrument_id']}:{d['timeframe']} count={d['count']} gap={d['default_gap']} coverage={d['coverage_pct']}% issues={','.join(d['issues'])}")
            if len(deficits) > 20:

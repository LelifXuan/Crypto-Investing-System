...
下载与启动
请在 GitHub Releases 中下载 V1.3 的 Windows Portable 压缩包，例如：

CryptoInvestingSystem-V1.3-win-x64-portable.zip
解压到一个普通可写目录，例如：
D:\Apps\CryptoInvestingSystem\
然后使用以下任一入口启动：
...
目录结构
V1.3 Portable 版建议保持如下结构：
CryptoInvestingSystem-Portable/
  TradingSystemLauncher.exe       # 图形化启动入口
  start_portable.bat              # 控制台启动入口
  README.md                       # 项目说明
  README_PORTABLE.md              # 便携版说明
  release_manifest.json           # 发布清单
  portable_runtime.lock.json      # 内嵌运行环境锁定信息
  requirements-portable.txt       # 便携版依赖锁定列表
  app/                            # 后端服务、前端静态页面与核心逻辑
  alembic/                        # 数据库迁移脚本
  scripts/                        # 预检、打包、清理和维护脚本
  runtime_env/                    # 内嵌运行环境，原则上不要手动修改
    python/
      python.exe
      python311.dll
      python311.zip
      Lib/site-packages
  runtime/                        # 首次启动后自动生成，可备份或清理
    config/portable.env
